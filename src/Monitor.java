...
public class Monitor {
	...
	public synchronized void pickUp(final int piTID) {
		...
		// since the philosophers are sitting in a circle, we go back to the beginning
		// of the list once we reach the end
		...
		while (true) {
			...
			if (stateOfPhilosophers[RightPhilosopher] != PhilosopherStates.EATING
					&& stateOfPhilosophers[LeftPhilosopher] != PhilosopherStates.EATING && (WaitToEat.peek() == piTID
							|| (WaitToEat.peek() != LeftPhilosopher + 1 && WaitToEat.peek() != RightPhilosopher + 1))) {
	 ...
	 * eating).
	 */
	// added the philosopher's ID as a parameter so that we can add her to the
	// queue.
	public synchronized void requestTalk(final int piTID) {
		...
		while (true) {
			...
			if (stateOfPhilosophers[piTID - 1] != PhilosopherStates.EATING && WaitToTalk.peek() == piTID
					&& !anyoneTalking) {
				anyoneTalking = true;
				// remove the philosopher from the queue
				WaitToTalk.remove();
